...
app.use("/api/review", userReviewRoute);
...
const fetchUserOrders = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching order details" }); // Proper error handling
  }
};
...
app.get("/api/count/pending", async (req, res) => {
  try {
    const pendingcount = await Orders.countDocuments({
      orderStatus: "pending",
    }); // Get the count of all users
...
app.get("/api/count/cancelled", async (req, res) => {
  try {
    const cancelledcount = await Orders.countDocuments({
...
app.delete("/api/staff/:id", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Staff deleted successfully", deletedStaff });
  } catch (error) {
